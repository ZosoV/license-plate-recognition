...
class LPRVocab:
  @staticmethod
  def create_vocab(train_list_path, val_list_path, specialCharPath, use_h_concat=False, use_oi_concat=False):
    [vocab, r_vocab, num_classes] = LPRVocab._create_standard_vocabs(train_list_path, val_list_path,specialCharPath)
    if use_h_concat:
  ...
  def _read_specials(filepath):
    ...
    with open(filepath, 'r') as file_:
      for line in file_:
        characters = characters.union(re.findall('(<[^>]*>|.)', line))
    return characters

  @staticmethod
  def _create_standard_vocabs(train_list_path, val_list_path, specialCharPath):
    chars = set().union(LPRVocab._char_range('A', 'Z')).union(LPRVocab._char_range('0', '9'))
    chars = chars.union(LPRVocab._read_specials(specialCharPath))
    chars = list(chars)
    chars.sort()
    chars.append('_')
